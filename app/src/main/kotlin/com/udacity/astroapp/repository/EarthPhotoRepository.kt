...
class EarthPhotoRepository(
    ...
    suspend fun getEarthPhotosByDate(date: String, forceRefresh: Boolean = false): List<EarthPhoto> {
        return withContext(Dispatchers.IO) {
            val cachedPhotos = dao.getEarthPhotosByDate(date)
            
            if (!forceRefresh && cachedPhotos.isNotEmpty() && isCacheValid(cachedPhotos.first().cacheTimestamp)) {
                cachedPhotos
            } else {
                try {
                    val networkPhotos = queryUtils.fetchEarthPhotosFromNetwork(date)
                    val photosWithTimestamp = networkPhotos.map { 
