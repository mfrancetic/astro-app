...
class ObservatoryRepository(
    ...
    suspend fun refreshObservatories(
        ...
        return withContext(Dispatchers.IO) {
            val cachedObservatories = dao.getObservatoriesNewerThan(
            ...
            if (!forceRefresh && cachedObservatories.isNotEmpty()) {
                ...
                try {
                    val networkObservatories = if (latitude != null && longitude != null) {
                        queryUtils.fetchObservatoriesFromNetwork(latitude, longitude)
                    } else {
                        queryUtils.fetchObservatoriesFromNetwork()
                    }
                    
                    val observatoriesWithTimestamp = networkObservatories.map { 
