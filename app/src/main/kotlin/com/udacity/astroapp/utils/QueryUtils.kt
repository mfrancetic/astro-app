...
class QueryUtils {
    ...
    companion object {
        ...
        private const val OBSERVATORY_BASE_URL = "https://maps.googleapis.com/maps/api/place/textsearch/json?query=planetarium&observatory"
        private const val OBSERVATORY_DETAILS_BASE_URL = "https://maps.googleapis.com/maps/api/place/details/json?"
    ...
    suspend fun fetchPhotoFromNetwork(date: String): Photo? = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun fetchAsteroidsFromNetwork(startDate: String? = null, endDate: String? = null): List<Asteroid> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun fetchEarthPhotosFromNetwork(date: String): List<EarthPhoto> = withContext(Dispatchers.IO) {
        try {
    ...
    ): List<Observatory> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun searchObservatoriesFromNetwork(query: String): List<Observatory> = withContext(Dispatchers.IO) {
        try {
            // For search, we would modify the base URL to include the search query
            val searchUrl = "https://maps.googleapis.com/maps/api/place/textsearch/json?query=$query"
    ...
    private fun createPhotoUrl(date: String): URL {
        val baseUri = Uri.parse(PHOTO_BASE_URL)
    ...
    private fun createAsteroidUrl(startDate: String?, endDate: String?): URL {
        val baseUri = Uri.parse(ASTEROID_BASE_URL)
        val uriBuilder = baseUri.buildUpon()
    ...
    private fun createEarthPhotoUrl(date: String): URL {
        val baseUri = Uri.parse(EARTH_PHOTO_BASE_URL)
        val uriBuilder = baseUri.buildUpon()
            .appendPath(date)
            .appendQueryParameter(API_PARAM, Secret.nasa_api_key)
            .build()
    ...
    private fun createEarthPhotoImageUrl(date: String, image: String): URL {
        val baseUri = Uri.parse(EARTH_PHOTO_IMAGE_BASE_URL)
    ...
    private fun createObservatoryURL(latitudeLongitude: String?, language: String): URL {
        val baseUri = Uri.parse(OBSERVATORY_BASE_URL)
        ...
        latitudeLongitude?.let {
            builder.appendQueryParameter(LOCATION_PARAM, it)
        }
    ...
    private fun createSearchObservatoryURL(query: String, language: String): URL {
        ...
        val baseUri = Uri.parse(searchUrl)
        val builder = baseUri.buildUpon()
    ...
    private fun createObservatoryDetailsUrl(placeId: String, language: String): URL {
        val baseUri = Uri.parse(OBSERVATORY_DETAILS_BASE_URL)
        val builder = baseUri.buildUpon()
            .appendQueryParameter(GOOGLE_API_KEY_PARAM, Secret.google_play_services_api_key)
    ...
    private fun parseEarthPhotosJson(jsonResponse: String, requestedDate: String): List<EarthPhoto> {
        if (jsonResponse.isEmpty()) return emptyList()
